			...
			System.out.println("欢迎使用答答租车系统／n您是否要租车：1是0否");
			 ...
			 int chance = scan.nextInt();
				...
				System.out.println("请输入汽车数目");	
				 ...
				 int[] rentNum = new int[carNum];
				 Bill bill = new Bill();
				 
				for(int i= 0;i<carNum;i++){
					System.out.println("请输入第"+(i+1)+"辆车的序号：");
					rentNum[i] = scan.nextInt();
					int num = rentNum[i];
					if(cars[num] instanceof PassengerCar){
						PassengerCar passenger = (PassengerCar)cars[i];
						
					}else if(cars[num] instanceof Trunk){
						Trunk trunk = (Trunk)cars[i];
						
					}else if(cars[num] instanceof PickUp){
						PickUp pickUp = (PickUp)cars[i];
